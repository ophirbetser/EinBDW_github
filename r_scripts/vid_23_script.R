...
calendar <- 
  fread(
    'https://raw.githubusercontent.com/ophirbetser/EinBDW_github/master/datasets/calendar_clean.csv'
  )

# take a look at it
calendar %>% head %>% datatable()
list %>% View()

# get 
# price_dollars, beds, host_since
# from 
# list
# to 
# calendar
...
merge(
  calendar,
  list[, .(listing_id = id, org_price = price_dollars, beds, host_since)],
  by = "listing_id"
)

merge(
  calendar,
  list[, .(id, price_dollars, beds, host_since)],
